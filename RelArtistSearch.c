...
int send_data_over_socket(int socket, void *buffer, uint64_t buffer_size);
int recv_data_over_socket(int socket, char *buffer, uint64_t buffer_size);
int recv_data_until_termination_string(int socket, char *buffer, uint64_t max_size);
int get_search_results(char *name, uint32_t temp, struct spotify_artist_result *result, uint64_t max_results);
struct spotify_artist_result* json_parser(char *jsonBytes, uint32_t length);
...
struct spotify_artist_result *result get_search_results(char *name, uint32_t name_length, struct spotify_artist_result *result, uint64_t max_results) {
  ...
  // return result
  char buf[10] = {0};
  int buf_size = recv_data_until_termination_string(socket_fd, buf, 10);
  if (buf_size == -1) {
    return NULL;
  }
